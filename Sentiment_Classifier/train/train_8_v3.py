...
preds = Dense(8, activation='softmax')(dense)

model = Model(sequence_input, preds)
adadelta = optimizers.Adadelta(lr=1.0, epsilon=None, decay=0.000)


model_checkpoints = callbacks.ModelCheckpoint(p+"/model/checkpoint-8labels-{val_loss:.3f}.h5", verbose=0,period=0,monitor='val_loss', save_best_only=True, save_weights_only=False, mode='auto')
model.summary()
...
model_log = model.fit(train_features_8,train_labels_8, validation_data=(val_features_8,val_labels_8),
