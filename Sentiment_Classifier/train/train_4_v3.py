...
preds = Dense(4, activation='softmax')(dense)
...
model_checkpoints = callbacks.ModelCheckpoint(p+"/model/checkpoint-4labels-{val_loss:.3f}.h5", verbose=0,period=0,monitor='val_loss', save_best_only=True, save_weights_only=False, mode='auto')
model.summary()
...
model_log = model.fit(train_features_4,train_labels_4, validation_data=(val_features_4,val_labels_4),
