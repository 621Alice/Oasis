...
lstm_1 = Bidirectional(LSTM(6,recurrent_dropout=0.0,return_sequences=True,dropout=0.1))(embedded_sequences)
...
lstm_3= Bidirectional(LSTM(6,dropout=0.1,recurrent_dropout=0.0,return_sequences=True))(merge_2)
...
lstm_4= Bidirectional(LSTM(6,dropout=0.1,recurrent_dropout=0.0,return_sequences=True))(merge_3)
conv_4 = Conv1D(filters=12,kernel_size=2,activation='relu',kernel_regularizer=regularizers.l2(0.0001))(lstm_4)
conv_4 = Dropout(0.1)(conv_3)
...
lstm_7 = Bidirectional(LSTM(6,dropout=0.1,recurrent_dropout=0.0,return_sequences=True))(conv_7)
...
lstm_8 = Bidirectional(LSTM(6,dropout=0.1,recurrent_dropout=0.0,return_sequences=True))(conv_8)
...
preds = Dense(12, activation='softmax')(dense)

model = Model(sequence_input, preds)
adadelta = optimizers.Adadelta(lr=1.0, epsilon=None, decay=0.000)


model_checkpoints = callbacks.ModelCheckpoint(p+"/model/checkpoint-12labels-{val_loss:.3f}.h5", verbose=0,period=0,monitor='val_loss', save_best_only=True, save_weights_only=False, mode='auto')
model.summary()
